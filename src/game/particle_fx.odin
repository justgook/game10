...
fx_land_smoke :: proc(pool: ^Particle_Pool, x, y: f32, intensity: f32 = 1.0) {
	...
	for i := 0; i < count; i += 1 {
		...
		p := particle_alloc(pool, .Main, .Normal, x + particle_rnd(0, 6) * dir, y + particle_rnd_centered(2))
		if p == nil {return}
...
fx_double_jump :: proc(pool: ^Particle_Pool, x, y: f32) {
	...
	for i := 0; i < 15; i += 1 {
		...
		p := particle_alloc(pool, .Main, .Additive, x + particle_rnd(0, 8) * dir, y - particle_rnd(0, 3)) // Below player
		if p == nil {return}
	...
	for i := 0; i < 10; i += 1 {
		...
		p := particle_alloc(pool, .Main, .Normal, x + particle_rnd(0, 5) * dir, y + particle_rnd_centered(2))
		if p == nil {return}
...
fx_gun_shot :: proc(pool: ^Particle_Pool, x, y: f32, dir: f32) {
	...
	if p != nil {
		...
		particle_set_fade(p, 1.0, 0, 0.5) // Slower fade
		particle_set_color_hex(p, 0xffb600) // Orange-yellow
		particle_set_color_anim_hex(p, 0xff4a00, 0.1) // Fade to red-orange
		particle_set_scale_xy(p, dir_mul * particle_rnd(1.5, 2.0), particle_rnd(1.2, 1.4))
		particle_set_life(p, 0.2) // Longer life
	}

	// Core sparks - 6 bright flashes
	for i := 0; i < 6; i += 1 {
		p := particle_alloc(pool, .Main, .Additive, x + particle_rnd_centered(1), y + particle_rnd_centered(1))
		if p == nil {return}

		particle_set_pivot(p, 0, 0.5)
		particle_set_alpha(p, particle_rnd(0.8, 1.0))
		particle_set_fade(p, p.alpha, 0, 0.5) // Slower fade
		particle_set_color_hex(p, 0xffb600)
		particle_set_color_anim_hex(p, 0xff4a00, particle_rnd(0, 0.1))
		particle_set_scale_xy(p, dir_mul * particle_rnd(0.7, 1.5), particle_rnd(1.5, 2.5))
		particle_set_life(p, particle_rnd(0.1, 0.15)) // Longer life
	}
	...
	for i := 0; i < n; i += 1 {
		...
		particle_set_fade(p, p.alpha, 0, 0.3) // Slower fade
		...
		particle_set_life(p, particle_rnd(0.15, 0.25)) // Longer life
	}
}
...
fx_hit_wall :: proc(pool: ^Particle_Pool, x, y: f32, normal_dir: f32) {
	...
	// Main impact flash
	p := particle_alloc(pool, .Main, .Additive, x - dir_mul * particle_rnd(1, 3), y + particle_rnd_centered(2))
	if p != nil {
		particle_set_pivot(p, 0, 0.5)
		particle_set_alpha(p, 1.0)
	...
	// Falling sparks
	for i := 0; i < 5; i += 1 {
		p := particle_alloc(pool, .Main, .Additive, x, y + particle_rnd_centered(2))
		if p == nil {return}
...
fx_cartridge :: proc(pool: ^Particle_Pool, x, y: f32, dir: f32) {
	...
	p := particle_alloc(pool, .Main, .Normal, x + particle_rnd_centered(1), y)
	if p == nil {return}

	life := particle_rnd(1.5, 2.8)
	particle_set_alpha(p, 1.0)
	particle_set_fade(p, 1.0, 0, 1.0 / life) // Slow fade
...
fx_blood_back :: proc(pool: ^Particle_Pool, x, y: f32, dir: f32, qty: f32 = 1.0) {
	...
	p := particle_alloc(pool, .Main, .Normal, x + dir_mul * particle_rnd_centered(1), y + particle_rnd_centered(4))
	if p != nil {
		particle_set_pivot(p, 0, 0.5)
	...
	for i := 0; i < count; i += 1 {
		p := particle_alloc(pool, .BG, .Normal, x + particle_rnd_centered(3), y + particle_rnd_centered(6))
		if p == nil {return}
