...
init :: proc "c" () {
	...
	buf = sshape.build_torus(buf, {radius = 0.5, ring_radius = 0.3, sides = 20, rings = 30})
	state.donut = sshape.element_range(buf)

	buf = sshape.build_sphere(buf, {radius = 0.5, slices = 72, stacks = 40})
	...
	smp := sg.make_sampler(
		{min_filter = .LINEAR, mag_filter = .LINEAR, wrap_u = .REPEAT, wrap_v = .REPEAT},
	)
