...
default_style := Style {
	...
	draw_titlebar = default_draw_titlebar,
	font = nil,
	size = {68, 10},
	padding = 5,
	spacing = 4,
...
default_draw_frame :: proc(ctx: ^Context, rect: Rect, colorid: Color_Type, id: Id) {
	draw_rect(ctx, rect, ctx.style.colors[colorid], id)
	if colorid == .SCROLL_BASE || colorid == .SCROLL_THUMB || colorid == .TITLE_BG {
		return
	}
	if ctx.style.colors[.BORDER].a != 0 { /* draw border */
		draw_box(ctx, add_padding(rect, 1), ctx.style.colors[.BORDER], id)
	}
}
...
default_draw_titlebar :: proc(ctx: ^Context, title: string, opt: Options, id: Id) {
	...
	if .NO_TITLE not_in opt {
		...
		draw_control_text(ctx, title, tr, .TITLE_TEXT, opt, id)
		if .NO_DRAG not_in opt && tid == ctx.focus_id && ctx.mouse_down_bits == {.LEFT} {
	...
	if .NO_CLOSE not_in opt {
		...
		tr.w -= r.w
		draw_icon(ctx, .CLOSE, r, ctx.style.colors[.TITLE_TEXT], id)
		update_control(ctx, cid, r, opt)
		if .LEFT in ctx.mouse_released_bits && cid == ctx.hover_id {
			// fmt.println("IMPLEMENT CLOSE ACTION")
			// cnt.open = false
		}
	}
}
