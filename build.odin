...
SOKOL_URL :: #config(SOKOL_URL, "https://github.com/floooh/sokol-odin/archive/refs/heads/main.tar.gz")
...
main :: proc() {
	...
	if args.release {
		...
		exe_path = must(build_hot(&args))
	}

	if exe_path != "" && args.run {
		fmt.printf("Starting %s\n", exe_path)
		if args.web {
			if err := execute("%s %s", WEB_SERVER, exe_path); err != nil {
			...
			if err := execute("./%s", file); err != nil {
				fmt.eprint("error during compile", err)
				os.exit(1)
			}
		}
	}
...
update_sokol :: proc() -> os.Error {
	...
	when ODIN_OS == .Linux {
		execute("chmod +x %s/linux/sokol-shdc", SOKOL_SHDC_PATH) or_return
		execute("chmod +x %s/linux_arm64/sokol-shdc", SOKOL_SHDC_PATH) or_return
	} else when ODIN_OS == .Darwin {
		execute("chmod +x %s/osx/sokol-shdc", SOKOL_SHDC_PATH) or_return
		execute("chmod +x %s/osx_arm64/sokol-shdc", SOKOL_SHDC_PATH) or_return
	}
...
compile_sokol :: proc(args: ^Args) -> os.Error {
	...
	when ODIN_OS == .Windows {
		...
		execute("build_clibs_windows.cmd") or_return

		switch {
		case emsdk_env != "":
			execute("%s && build_clibs_wasm.bat", emsdk_env) or_return
		case which("emcc.bat") != "":
			execute("build_clibs_wasm.bat") or_return
		case:
			fmt.println(
		...
		when ODIN_OS == .Linux {
			execute("./build_clibs_linux.sh") or_return
		} else when ODIN_OS == .Darwin {
			execute("./build_clibs_macos.sh && ./build_clibs_macos_dylib.sh") or_return
		}

		switch {
		case emsdk_env != "":
			os.set_env("EMSDK_QUIET", "1") or_return
			execute("%s && ./build_clibs_wasm.sh", emsdk_env) or_return
		case which("emcc") != "":
			execute("./build_clibs_wasm.sh") or_return
		case:
			fmt.println(
...
build_release :: proc(args: ^Args) -> (out: string, err: os.Error) {
	...
	append(
		&cmd,
		..[]string{"odin", "build", fmt.tprintf("%s/release", ENTRY_SRC_DIR), fmt.tprintf("-out:%s", exe)},
	)
	append_exe_build_args(&cmd, args, "main,game")
...
build_web :: proc(args: ^Args) -> (out: string, err: os.Error) {
	...
	append(
		...
		..[]string {
			...
			"-target:js_wasm32",
			"-build-mode:obj",
			fmt.tprintf("-out:%s/game.wasm.o", WEB_OUT),
		},
	)

	append_exe_build_args(&cmd, args, "main")
	execute_slice(cmd[:]) or_return
	...
	emcc_command := fmt.tprintf("emcc %s -o %s/index.html %s %s", build_flags, WEB_OUT, emcc_files_str, emcc_flags)
	...
	execute(emcc_command) or_return
...
@(require_results)
build_hot :: proc(args: ^Args) -> (out: string, err: os.Error) {
	...
	append_exe_build_args(&cmd, args, "game")
...
append_exe_build_args :: proc(cmd: ^[dynamic]string, args: ^Args, vet_packages: string) {
	if args.debug {
		append(cmd, "-debug")
	} else {
		if args.web || args.release {
...
get_emscripten_env_command :: proc(emsdk_path: string) -> (out: string, err: os.Error) {
	...
	when ODIN_OS == .Windows {
		path := os.join_path([]string{emsdk_path, "emsdk_env.bat"}, context.temp_allocator) or_return
		if !os.exists(path) {
			return "", .Not_Exist
		}
		return path, nil
	} else when ODIN_OS == .Linux || ODIN_OS == .Darwin {
		path := os.join_path([]string{emsdk_path, "emsdk_env.sh"}, context.temp_allocator) or_return
		if !os.exists(path) {
			return "", .Not_Exist
		}
...
execute :: proc(input: string, args: ..any) -> os.Error {
	...
	when ODIN_OS == .Windows {
		execute_slice([]string{cmd}) or_return
	} else when ODIN_OS == .Linux || ODIN_OS == .Darwin {
		execute_slice([]string{"bash", "-c", cmd}) or_return
	}
...
@(require_results)
execute_slice :: proc(cmd: []string) -> (err: os.Error) {
...
download :: proc(url: string, output_path: string) -> os.Error {
	execute("curl -# -L -o %s %s", output_path, url) or_return
...
delete_dir :: proc(name: string) -> os.Error {
	execute_slice([]string{"bash", "-c", fmt.tprintf("rm -rf %s || true", name)}) or_return
